...
public class BrandController {
    ...
    @Operation(summary = "브랜드별 특정 레벨 조회", description = "특정 브랜드의 특정 레벨 정보를 조회합니다")
    @ApiResponse(responseCode = "200", description = "브랜드별 레벨 조회 성공")
    @GetMapping("/{brandId}/levels/{levelId}")
    public ResponseEntity<ApiResult<GymLevelResponse>> getBrandLevel(
            @Parameter(description = "브랜드 ID") @PathVariable Long brandId,
            @Parameter(description = "암장별 레벨 ID (GymLevel ID)") @PathVariable Long levelId) {
        return ResponseEntity.ok(ApiResult.success(brandService.getBrandLevel(brandId, levelId)));
    }
}